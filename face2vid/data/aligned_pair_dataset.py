...
class AlignedPairDataset(BaseDataset):
    def initialize(self, opt):
        ...
        #self.dir_A = os.path.join(opt.dataroot, "test" + dir_A)
        self.dir_A = os.path.join(opt.dataroot)
        #Utils.create_folder(self.dir_A)
        self.A_paths = sorted(make_dataset(self.dir_A))

        #dir_B = '_B' #if self.opt.label_nc == 0 else '_img'
        #self.dir_B = os.path.join(opt.dataroot, "test" + dir_B)
        self.dir_B = os.path.join(opt.dataroot)
        #Utils.create_folder(self.dir_B)
