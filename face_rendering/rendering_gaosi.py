...
def render(facemodel, chi, mask3, triangles):
    ...
    face_color, _ = Illumination_layer(np.expand_dims(colors, 0), face_norm_r, gamma)
    colors = face_color[0,:]
...
def rendering(uid, selected_model_str, npz_path):
    #train_params_path = f'/home/dilab05/work_directory/capstone/{folder}/video_preprocess/train1_posenew.npz'
    #net_params_path = f'/home/dilab05/work_directory/capstone/{folder}/examples/test-result/test1.npz'
    #outpath = f'/home/dilab05/work_directory/capstone/{folder}/examples/rendering/'
    train_params_path = f'{Constants.DEFAULT_NEED_FILES}/{selected_model_str}/{Constants.DEFAULT_TRAIN_PARAM}'
    #net_params_path = f'{Constants.DEFAULT_USER_DIRECTORY}/{uid}/'
    net_params_path = npz_path
    eyemask_path = f'{Constants.DEFAULT_NEED_FILES}/{selected_model_str}/{Constants.DEFAULT_EYEMASK_PATH}'
    outpath = f'{Constants.DEFAULT_USER_DIRECTORY}/{uid}/rendering'
    
    mask3 = np.load(eyemask_path)
    #gaosifilter
    r = 3
    sigma = 1
    ...
    # --- 1. load model
    bfm_model_path = f'{Constants.DEFAULT_NEED_FILES}/{selected_model_str}/{Constants.DEFAULT_BFM_FILE}'
    bfm_std_exp_txt = f'{Constants.DEFAULT_NEED_FILES}/{selected_model_str}/{Constants.DEFAULT_BFM_STD_EXP_TXT}'
    facemodel = BFM(bfm_model_path, bfm_std_exp_txt)
    ...
    #save_folder = outpath + examplename
    #if not os.path.exists(save_folder):
    #    os.makedirs(save_folder)
    if not os.path.exists(outpath):
        os.makedirs(outpath)
    #if not os.path.exists(outpath + "/test_B"):
    #    os.makedirs(outpath + "/test_B")

    logger(f"Rendering gaosi started. | Request from {uid} | Processing {selected_model_str}", level="EXE_START")
    for i in range(1,netparams.shape[0]+1):
        ...
        image = render(facemodel, chi_next, mask3, triangles).astype(np.uint8)
        #io.imsave(os.path.join(save_folder,str("%06d"%(i))+'.jpg'), image)
        io.imsave(os.path.join(outpath, str("%06d"%(i))+'.jpg'), image)
        #io.imsave(os.path.join(outpath,"test_B",str("%06d"%(i))+'.jpg'), image)
    
    logger(f"Gaosi Render Successfully finished. | Request from {uid} | Processing {selected_model_str}", level="EXE_FINISH")